...
export class SalesInvoicesService {
  ...
  async update(id: string, dto: UpdateSalesInvoiceDto): Promise<SalesInvoice & { items: SalesInvoiceItem[] }> {
    ...
    const oldItems = await itemRepo.find({ where: { salesInvoiceId: id, tenantId } });

    // Reverse the stock deduction the old line items made, then re-apply for the new ones.
    for (const oldLine of oldItems) {
      ...
      await itemMasterRepo.increment({ id: oldLine.itemId, tenantId }, 'currentStock', parseFloat(oldLine.qty));
    }
    await itemRepo.delete({ salesInvoiceId: id, tenantId });
    ...
    for (const line of lines) {
      ...
      await itemMasterRepo.decrement({ id: line.itemId, tenantId }, 'currentStock', line.qty);
    }
    ...
    const items = await itemRepo.find({ where: { salesInvoiceId: id }, order: { lineNo: 'ASC' } });
    await this.auditLog.record({ action: 'sales_invoice.updated', entityType: 'sales_invoice', entityId: id, before: invoice, after });
    return { ...after, items };
  }
  ...
  async remove(id: string): Promise<{ id: string }> {
    ...
    const stockRepo = manager.getRepository(StockLedger);
    const itemMasterRepo = manager.getRepository(Item);
    const items = await itemRepo.find({ where: { salesInvoiceId: id, tenantId } });
    ...
    for (const line of items) {
      ...
      await itemMasterRepo.increment({ id: line.itemId, tenantId }, 'currentStock', parseFloat(line.qty));
    }

    await manager.getRepository(SalesInvoice).delete({ id, tenantId });
    await this.auditLog.record({ action: 'sales_invoice.deleted', entityType: 'sales_invoice', entityId: id, before: invoice });
    return { id };
  }
