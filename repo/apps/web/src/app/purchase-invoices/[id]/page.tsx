...
export default function PurchaseInvoiceDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [payment, setPayment] = useState({ amount: "", paidDate: todayIso(), paymentMode: "cash" as PaymentMode, referenceNo: "" });
  ...
  async function handlePayment(e: React.FormEvent) {
    ...
    try {
      ...
      setPayment({ amount: "", paidDate: todayIso(), paymentMode: "cash", referenceNo: "" });
      load();
    } catch (err) {
      ...
      <div className="mx-auto flex max-w-3xl flex-col gap-6">
        ...
        <div className="flex justify-end gap-2 print:hidden">
          ...
          <button
            onClick={async () => {
              try {
                setDownloading(true);
                await downloadInvoicePdf("purchase", invoice.id);
              } catch {
          ...
          <button
            ...
            onMouseEnter={() => prefetchInvoicePdf("purchase", invoice.id)}
            onFocus={() => prefetchInvoicePdf("purchase", invoice.id)}
            onTouchStart={() => prefetchInvoicePdf("purchase", invoice.id)}
            disabled={sharing}
            aria-busy={sharing}
        ...
        <Card className="overflow-hidden !p-0">
          ...
          <div className="flex flex-wrap items-start justify-between gap-4 border-b border-slate-200 bg-gradient-to-r from-emerald-50 to-slate-50 px-6 py-5 dark:border-slate-700 dark:from-emerald-950/30 dark:to-transparent">
            <div>
              <p className="text-[11px] font-semibold uppercase tracking-widest text-emerald-700 dark:text-emerald-400">Purchase Invoice</p>
