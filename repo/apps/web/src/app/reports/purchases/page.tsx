...
export default function PurchasesReportPage() {
  ...
  useEffect(() => {
    api.getPurchasesReport({ from, to, partyId: partyId || undefined }).then(setRows).catch(() => null);
  }, [from, to, partyId]);

  const total = rows.reduce((s, r) => s + parseFloat(r.total_amount), 0);
  const paid  = rows.reduce((s, r) => s + parseFloat(r.paid_amount), 0);

  function handleDownload() {
    downloadStyledExcel({
      filename: `purchase-report-${from}-to-${to}.xlsx`,
      ...
      columns: [
        ...
        { header: "Supplier", key: "supplier", width: 24, type: "text" },
  ...
  return (
    <AppShell title="Purchase Report">
      <div className="mb-4 rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
        ...
        <div className="mt-4 flex flex-col gap-3 border-t border-slate-100 pt-4 sm:flex-row sm:items-center sm:justify-between">
          <div className="flex flex-wrap gap-2 text-sm">
            <span className="rounded-lg bg-slate-50 px-3 py-1.5 text-slate-500">Total <strong className="ml-1 text-slate-800" title={formatINR(total)}>{formatCompactINR(total)}</strong></span>
            <span className="rounded-lg bg-slate-50 px-3 py-1.5 text-slate-500">Paid <strong className="ml-1 text-emerald-700" title={formatINR(paid)}>{formatCompactINR(paid)}</strong></span>
            <span className="rounded-lg bg-slate-50 px-3 py-1.5 text-slate-500">Pending <strong className="ml-1 text-orange-700" title={formatINR(total - paid)}>{formatCompactINR(total - paid)}</strong></span>
          </div>
      ...
      <div className={tableWrapClass}>
        <table className="w-full text-sm">
          <thead><tr>
            {["Invoice No", "Date", "Supplier", "Total", "Paid", "Balance", "Status"].map(h => <th key={h} className={thClass}>{h}</th>)}
          </tr></thead>
          <tbody>
            {rows.length === 0 && <tr><td colSpan={7} className={tdClass + " text-center text-slate-400"}>No purchases in this period</td></tr>}
            {rows.map(r => (
              <tr key={r.id} className="hover:bg-slate-50">
                <td className={tdClass}><Link href={`/purchase-invoices/${r.id}`} className="text-emerald-700 hover:underline">{r.invoice_no}</Link></td>
